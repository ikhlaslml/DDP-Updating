...
lines.push(`  kecamatan String? // administrative profile, kept separate from letterhead copy`);
lines.push(`  kabupatenKota String?`);
lines.push(`  provinsi String?`);
lines.push(`  tahunPendataan Int? // authoritative baseline/data-collection year for this tenant`);
...
lines.push(`  wilayahDeskel String?`);
lines.push(`  wilayahKecamatan String?`);
lines.push(`  wilayahKabkota String?`);
lines.push(`  wilayahProvinsi String?`);
lines.push(`  wilayahKodeDeskel String?`);
lines.push(`  createdBy      String?`);
lines.push(`  createdByName  String?`);
lines.push(`  createdByEmail String?`);
lines.push(`  createdAt      DateTime @default(now())`);
...
lines.push(`  scope      String   @default("PERSON")`);
lines.push(`  source     String   @default("EDIT")`);
lines.push(`  updatedById String?`);
lines.push(`  updatedByName String?`);
lines.push(`  updatedByEmail String?`);
lines.push(`  stagingChangeId String?`);
...
lines.push(`  @@index([desaId, nkk, scope, field])`);
lines.push(`  @@index([stagingChangeId])`);
lines.push(`}`);
lines.push(``);
lines.push(`// Append-only audit history for edits and no-change confirmations.`);
lines.push(`model FieldUpdateLog {`);
lines.push(`  id         String   @id @default(cuid())`);
lines.push(`  desaId     String`);
lines.push(`  pendudukId String`);
lines.push(`  nkk        String?`);
...
lines.push(`  createdAt  DateTime @default(now())`);
lines.push(``);
lines.push(`  @@index([desaId, field, createdAt])`);
lines.push(`  @@index([desaId, nkk, scope])`);
lines.push(`  @@index([pendudukId])`);
lines.push(`  @@index([stagingChangeId])`);
lines.push(`}`);
lines.push(``);
...
lines.push(`}`);
lines.push(``);
lines.push(`// Physical building removed from the active spatial registry. Residents and`);
lines.push(`// family history remain intact; this is an auditable tombstone, not a cascade.`);
lines.push(`model BangunanDihapus {`);
lines.push(`  id            String   @id @default(cuid())`);
lines.push(`  desaId        String`);
lines.push(`  kodeBangunan  Int`);
lines.push(`  alasan        String`);
lines.push(`  keterangan    String?`);
lines.push(`  stagingChangeId String?`);
lines.push(`  deletedBy     String`);
lines.push(`  deletedByName String?`);
lines.push(`  deletedByEmail String?`);
lines.push(`  deletedAt     DateTime @default(now())`);
lines.push(``);
lines.push(`  @@unique([desaId, kodeBangunan])`);
lines.push(`  @@index([desaId, deletedAt])`);
lines.push(`  @@index([stagingChangeId])`);
lines.push(`}`);
lines.push(``);
...
lines.push(`  logoUrl    String?`);
lines.push(`  logoMediaAssetId String?`);
lines.push(`  logoUpdatedAt DateTime?`);
lines.push(`  tandaTanganUrl String?`);
lines.push(`  tandaTanganMediaAssetId String?`);
lines.push(`  tandaTanganUpdatedAt DateTime?`);
lines.push(`  tanggalBaselineData DateTime?`);
lines.push(`  updatedAt  DateTime @updatedAt`);
lines.push(``);
lines.push(`  @@index([desaId, logoMediaAssetId])`);
lines.push(`  @@index([desaId, tandaTanganMediaAssetId])`);
lines.push(`}`);
lines.push(``);
lines.push(`// Master catalog derived verbatim (trimmed) from Borang Harga.xlsx.`);
lines.push(`model Komoditas {`);
lines.push(`  id        String   @id`);
lines.push(`  kode      String   @unique`);
lines.push(`  kategori  String`);
lines.push(`  nama      String`);
lines.push(`  satuan    String`);
lines.push(`  urutan    Int`);
lines.push(`  aktif     Boolean  @default(true)`);
lines.push(`  createdAt DateTime @default(now())`);
lines.push(`  updatedAt DateTime @updatedAt`);
lines.push(``);
lines.push(`  @@index([kategori, urutan])`);
lines.push(`}`);
lines.push(``);
lines.push(`// Tenant-scoped commodity price history: one value per commodity and period.`);
lines.push(`model HargaKomoditas {`);
lines.push(`  id            String   @id @default(cuid())`);
lines.push(`  desaId        String`);
lines.push(`  komoditasId   String`);
lines.push(`  periode       String`);
lines.push(`  harga         ${DB_PROVIDER === "postgresql" ? "Decimal @db.Decimal(18, 2)" : "Decimal"}`);
lines.push(`  sumberData    String`);
lines.push(`  createdBy     String`);
lines.push(`  createdByName String?`);
lines.push(`  updatedBy     String`);
lines.push(`  updatedByName String?`);
...
lines.push(`  updatedAt     DateTime @updatedAt`);
lines.push(``);
lines.push(`  @@unique([desaId, komoditasId, periode])`);
lines.push(`  @@index([desaId, periode])`);
lines.push(`  @@index([komoditasId])`);
lines.push(`}`);
lines.push(``);
...
lines.push(`  tahunNomor    Int?`);
lines.push(`  urutanNomor   Int?`);
lines.push(`  isiSnapshot   String?`);
lines.push(`  pengaturanSnapshot String?`);
lines.push(`  wargaSnapshot String?`);
lines.push(`  issuedBy      String?`);
lines.push(`  issuedByName  String?`);
lines.push(`  issuedByEmail String?`);
lines.push(`  createdAt    DateTime @default(now())`);
lines.push(``);
lines.push(`  @@index([createdAt])`);
lines.push(`  @@index([desaId])`);
lines.push(`  @@index([desaId, peristiwaId])`);
lines.push(`  @@unique([desaId, tahunNomor, urutanNomor])`);
lines.push(`}`);
lines.push(``);
lines.push(`// Atomic yearly sequence per tenant for letter numbering.`);
lines.push(`model NomorSuratCounter {`);
lines.push(`  id          String   @id @default(cuid())`);
lines.push(`  desaId      String`);
lines.push(`  tahun       Int`);
lines.push(`  nomorTerakhir Int    @default(0)`);
lines.push(`  updatedAt   DateTime @updatedAt`);
lines.push(``);
lines.push(`  @@unique([desaId, tahun])`);
lines.push(`}`);
lines.push(``);
