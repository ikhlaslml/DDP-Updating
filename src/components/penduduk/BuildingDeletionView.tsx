...
export function BuildingDeletionView() {
  ...
  async function submit() {
    ...
    try {
      ...
      const json = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(json.error ?? "Penghapusan bangunan tidak dapat dicatat.");
      router.push("/penduduk?staged=hapus-bangunan");
      router.refresh();
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "Penghapusan bangunan tidak dapat dicatat.");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-2xl border border-rose-100 bg-white p-5 shadow-sm sm:p-6">
        <div className="flex items-start gap-3"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-rose-50 text-rose-600"><Trash2 className="h-5 w-5" /></span><div><h2 className="text-lg font-bold text-slate-900">Pilih Bangunan yang Akan Dihapus</h2></div></div>
        {!canWrite ? <p className="mt-4 rounded-xl bg-amber-50 px-4 py-3 text-sm text-amber-800">Pengajuan penghapusan bangunan hanya dapat dilakukan petugas desa.</p> : null}
        <div className="mt-5 grid items-end gap-3 lg:grid-cols-[minmax(0,1fr)_20rem]">
          <label className="block text-xs font-semibold text-slate-500">
            Cari
            <span className="relative mt-1.5 block">
              <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
              <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Kode, kategori, atau lokasi" className="min-h-11 w-full rounded-xl border border-slate-300 py-2 pl-10 pr-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </span>
          </label>
          <label className="block text-xs font-semibold text-slate-500">
            Bangunan
            <select value={selectedCode} onChange={(event) => { setSelectedCode(event.target.value); setConfirmed(false); }} className="mt-1.5 min-h-11 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm">
              <option value="">Pilih bangunan</option>
              {filtered.map((building) => <option key={building.kode} value={building.kode}>#{building.kode} — {building.kategori ?? building.jenis.replaceAll("_", " ")} — {location(building)}</option>)}
            </select>
          </label>
        </div>
      </section>

      {selected ? <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Bangunan yang dipilih</p><h2 className="mt-1 flex items-center gap-2 text-xl font-bold text-slate-900"><Building2 className="h-5 w-5 text-indigo-600" /> Bangunan #{selected.kode}</h2><p className="mt-1 text-sm text-slate-600">{selected.kategori ?? selected.jenis.replaceAll("_", " ")}{selected.legacy ? " · data awal" : ""}</p></div><div className="grid grid-cols-2 gap-2 text-center text-sm"><div className="rounded-xl bg-slate-50 px-4 py-3"><UsersRound className="mx-auto mb-1 h-4 w-4 text-indigo-600" /><strong className="block text-slate-900">{selected.jumlahKk}</strong><span className="text-xs text-slate-500">KK</span></div><div className="rounded-xl bg-slate-50 px-4 py-3"><strong className="block text-slate-900">{selected.jumlahPenduduk}</strong><span className="text-xs text-slate-500">warga</span></div></div></div><p className="mt-4 flex items-start gap-2 rounded-xl bg-slate-50 px-4 py-3 text-sm text-slate-700"><MapPin className="mt-0.5 h-4 w-4 shrink-0 text-indigo-600" /> {location(selected)}</p>
        ...
        <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4"><div className="flex items-start gap-2"><AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-amber-700" /><div><p className="font-semibold text-amber-900">Penghapusan tidak menghapus data penduduk.</p><p className="mt-1 text-sm text-amber-800">Setelah perubahan diterapkan, bangunan tidak lagi tampil pada peta dan daftar bangunan aktif. {selected.jumlahKk} KK serta {selected.jumlahPenduduk} penduduk tetap tersimpan sebagai riwayat. Pastikan perubahan tempat tinggal setiap keluarga sudah dicatat sebelum melanjutkan.</p></div></div><label className="mt-4 flex cursor-pointer items-start gap-2 text-sm text-amber-950"><input type="checkbox" checked={confirmed} disabled={!canWrite || !reason} onChange={(event) => setConfirmed(event.target.checked)} className="mt-0.5 h-4 w-4 rounded border-amber-400 text-rose-600 focus:ring-rose-500" /> Saya memahami bahwa yang dihapus adalah bangunan fisik dari peta aktif, bukan data penduduk.</label></div>
        ...
        <div className="mt-5 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><button type="button" onClick={() => router.push("/penduduk")} className="min-h-11 rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">Batal</button>{canWrite ? <button type="button" disabled={!reason || !confirmed || submitting} onClick={() => void submit()} className="inline-flex min-h-11 items-center justify-center rounded-xl bg-rose-600 px-4 py-2 text-sm font-semibold text-white hover:bg-rose-700 disabled:cursor-not-allowed disabled:opacity-50">{submitting ? "Menyimpan..." : "Hapus Bangunan"}</button> : null}</div>
      </section> : null}
