  ...
  return (
    <div className="space-y-5">
      <nav aria-label="Bagian administrasi desa" className="grid grid-cols-1 gap-1 rounded-xl border border-slate-200 bg-slate-50 p-1 sm:inline-grid sm:grid-cols-2">
        <button type="button" onClick={() => changeMenu("identitas")} className={`inline-flex min-h-11 items-center justify-center rounded-lg px-4 py-2 text-sm font-semibold ${menu === "identitas" ? "bg-white text-indigo-700 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>
          Identitas dan Surat
        </button>
        <button type="button" onClick={() => changeMenu("harga-komoditas")} className={`inline-flex min-h-11 items-center justify-center rounded-lg px-4 py-2 text-sm font-semibold ${menu === "harga-komoditas" ? "bg-white text-indigo-700 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>
          Harga Komoditas
        </button>
      </nav>
