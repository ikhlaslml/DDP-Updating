  ...
  return (
    <div className="space-y-5">
      <div className="inline-flex w-full gap-1 rounded-xl border border-slate-200 bg-slate-50 p-1 sm:w-auto">
        <button type="button" onClick={() => changeTab("terbitkan")} className={`flex min-h-11 flex-1 items-center justify-center rounded-lg px-4 py-2 text-sm font-semibold sm:flex-none ${tab === "terbitkan" ? "bg-white text-indigo-700 shadow-sm" : "text-slate-500"}`}>Terbitkan</button>
        <button type="button" onClick={() => changeTab("riwayat")} className={`flex min-h-11 flex-1 items-center justify-center rounded-lg px-4 py-2 text-sm font-semibold sm:flex-none ${tab === "riwayat" ? "bg-white text-indigo-700 shadow-sm" : "text-slate-500"}`}>Riwayat</button>
      </div>
      ...
      {tab === "terbitkan" ? (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          {previewOpen && selected && template ? <section className="lg:col-span-2 rounded-2xl border border-indigo-200 bg-indigo-50/40 p-3 sm:p-5">
            <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h2 className="text-lg font-bold text-slate-900">3. Pratinjau dan Periksa</h2></div><button type="button" onClick={() => { setPreviewOpen(false); setValidated(false); }} className="inline-flex min-h-10 items-center justify-center rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">Batal</button></div>
            ...
            {canWrite ? <div className="mt-4 flex flex-col items-stretch gap-3 sm:flex-row sm:items-center sm:justify-between"><label className="flex min-h-11 cursor-pointer items-center gap-2 rounded-xl border border-indigo-200 bg-white px-3 py-2 text-sm text-slate-700"><input type="checkbox" checked={validated} onChange={(event) => setValidated(event.target.checked)} className="h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" /> Saya sudah memeriksa isi surat.</label><button type="button" disabled={!validated || issuing} onClick={issue} className="inline-flex min-h-11 items-center justify-center rounded-xl bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-50">{issuing ? "Menerbitkan..." : "Terbitkan"}</button></div> : null}
          </section> : null}

          {nomor && suratId ? <section className="lg:col-span-2 rounded-2xl border border-emerald-200 bg-emerald-50/50 p-3 sm:p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-lg font-bold text-emerald-900">Surat telah diterbitkan</h2><p className="mt-1 text-sm text-emerald-800">Nomor surat: <strong>{nomor}</strong>. Surat ini sudah masuk Riwayat Surat.</p></div><div className="flex flex-col gap-2 sm:flex-row"><button type="button" aria-label="Cetak" title="Cetak" onClick={() => window.open(`/api/surat/${suratId}/pdf?mode=inline`, "_blank", "noopener,noreferrer")} className="inline-flex min-h-11 w-11 items-center justify-center rounded-lg border border-slate-300 bg-white text-slate-700 hover:bg-slate-50"><Printer className="h-4 w-4" /></button><a href={`/api/surat/${suratId}/pdf`} aria-label="Unduh" title="Unduh" className="inline-flex min-h-11 w-11 items-center justify-center rounded-lg border border-emerald-300 bg-white text-emerald-700 hover:bg-emerald-50"><Download className="h-4 w-4" /></a></div></div>
