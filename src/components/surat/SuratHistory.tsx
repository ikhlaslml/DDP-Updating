...
export function SuratHistory({ templates }: { templates: Template[] }) {
  ...
  return (
    <section className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm sm:p-6">
      ...
      {(query || dateFrom || dateTo || templateId) ? <button type="button" onClick={clearFilters} className="mt-3 text-xs font-semibold text-indigo-600 hover:underline">Hapus pencarian</button> : null}
