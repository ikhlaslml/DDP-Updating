...
export default function MigrasiKeluarPage() {
  return <div className="space-y-6"><div><Link href="/penduduk" className="text-sm font-medium text-indigo-600 hover:underline">&larr; Kembali ke Data Kependudukan</Link><h1 className="mt-2 text-2xl font-bold text-slate-900">Catat Migrasi Keluar</h1><p className="mt-1 max-w-3xl text-sm text-slate-500">Data penduduk tetap tersimpan sebagai riwayat, tetapi tidak lagi dihitung sebagai penduduk aktif setelah perubahan diterapkan.</p></div><DemographicExitView type="MIGRASI_KELUAR" /><DemographicHistory mode="EVENT" /></div>;
}
