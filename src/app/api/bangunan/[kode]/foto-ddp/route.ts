...
export async function GET(_: Request, { params }: { params: Promise<{ kode: string }> }) {
  ...
  if (!building && !legacy && !staged.some((row) => stagedHasCode(row.data, code))) {
    return NextResponse.json({ error: "Bangunan tidak ditemukan di desa ini" }, { status: 404 });
  }
  if (!desa?.kodeWilayah) return NextResponse.json({ error: "Kode wilayah desa belum diisi" }, { status: 404 });

  try {
    const metadata = await fetchDdpBuildingPhoto(code, desa.kodeWilayah);
    if (!metadata) return NextResponse.json({ error: "Foto bangunan belum tersedia" }, { status: 404 });
    const image = await fetch(metadata.photoUrl, { cache: "no-store", signal: AbortSignal.timeout(15_000) });
    if (!image.ok) throw new Error("Foto bangunan tidak dapat diambil saat ini");
    const contentType = image.headers.get("content-type") ?? "";
    if (!contentType.startsWith("image/")) throw new Error("Berkas foto bukan gambar");
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "Foto bangunan tidak dapat dimuat" }, { status: 502 });
  }
}
